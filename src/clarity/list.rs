...
pub struct ListCV(u8, Vec<Box<dyn ClarityValue>>);

impl ListCV {
    pub fn new(values: Vec<Box<dyn ClarityValue>>) -> ListCV {
...
impl PartialEq for ListCV {
    fn eq(&self, other: &Self) -> bool {
        let own: Vec<Vec<u8>> = self.1.iter().map(|x| x.serialize()).collect();
        let other: Vec<Vec<u8>> = other.1.iter().map(|x| x.serialize()).collect();
        own == other
    }
}
...
impl DeserializeCV for ListCV {
    ...
    fn deserialize(bytes: &[u8]) -> Result<Self, Self::Err> {
        ...
        for _ in 0..len {
            ...
            let value: Box<dyn ClarityValue> = match type_id {
...
mod tests {
    ...
    fn test_list_cv() {
        ...
        let deserialized = ListCV::deserialize(&cv.serialize()).unwrap();
